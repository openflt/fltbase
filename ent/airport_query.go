...
func (aq *AirportQuery) QueryState() *StateQuery {
	query := (&StateClient{config: aq.config}).Query()
...
func (aq *AirportQuery) QueryCity() *CityQuery {
...
func (aq *AirportQuery) WithState(opts ...func(*StateQuery)) *AirportQuery {
	query := (&StateClient{config: aq.config}).Query()
...
func (aq *AirportQuery) WithCity(opts ...func(*CityQuery)) *AirportQuery {
	query := (&CityClient{config: aq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (aq *AirportQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Airport, error) {
	...
	if aq.withState != nil || aq.withCity != nil || aq.withRegion != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := aq.withState; query != nil {
		if err := aq.loadState(ctx, query, nodes, nil,
			func(n *Airport, e *State) { n.Edges.State = e }); err != nil {
			return nil, err
		}
	}
	if query := aq.withCity; query != nil {
		if err := aq.loadCity(ctx, query, nodes, nil,
			func(n *Airport, e *City) { n.Edges.City = e }); err != nil {
			return nil, err
		}
	}
...
func (aq *AirportQuery) loadState(ctx context.Context, query *StateQuery, nodes []*Airport, init func(*Airport), assign func(*Airport, *State)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Airport)
	for i := range nodes {
		if nodes[i].state_airports == nil {
			continue
		}
		fk := *nodes[i].state_airports
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "state_airports" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (aq *AirportQuery) loadCity(ctx context.Context, query *CityQuery, nodes []*Airport, init func(*Airport), assign func(*Airport, *City)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Airport)
	for i := range nodes {
		if nodes[i].city_airports == nil {
			continue
		}
		fk := *nodes[i].city_airports
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "city_airports" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
