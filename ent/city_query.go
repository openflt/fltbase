...
func (cq *CityQuery) QueryState() *StateQuery {
	query := (&StateClient{config: cq.config}).Query()
...
func (cq *CityQuery) QueryAirports() *AirportQuery {
...
func (cq *CityQuery) WithState(opts ...func(*StateQuery)) *CityQuery {
	query := (&StateClient{config: cq.config}).Query()
...
func (cq *CityQuery) WithAirports(opts ...func(*AirportQuery)) *CityQuery {
	query := (&AirportClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CityQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*City, error) {
	...
	if query := cq.withState; query != nil {
		if err := cq.loadState(ctx, query, nodes, nil,
			func(n *City, e *State) { n.Edges.State = e }); err != nil {
			return nil, err
		}
	}
	if query := cq.withAirports; query != nil {
		if err := cq.loadAirports(ctx, query, nodes,
			func(n *City) { n.Edges.Airports = []*Airport{} },
			func(n *City, e *Airport) { n.Edges.Airports = append(n.Edges.Airports, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range cq.withNamedAirports {
		if err := cq.loadAirports(ctx, query, nodes,
			func(n *City) { n.appendNamedAirports(name) },
			func(n *City, e *Airport) { n.appendNamedAirports(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (cq *CityQuery) loadState(ctx context.Context, query *StateQuery, nodes []*City, init func(*City), assign func(*City, *State)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*City)
	for i := range nodes {
		if nodes[i].state_cities == nil {
			continue
		}
		fk := *nodes[i].state_cities
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "state_cities" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (cq *CityQuery) loadAirports(ctx context.Context, query *AirportQuery, nodes []*City, init func(*City), assign func(*City, *Airport)) error {
	...
	query.Where(predicate.Airport(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(city.AirportsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.city_airports
		if fk == nil {
			return fmt.Errorf(`foreign-key "city_airports" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "city_airports" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (cq *CityQuery) WithNamedAirports(name string, opts ...func(*AirportQuery)) *CityQuery {
	query := (&AirportClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
